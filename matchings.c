...
int insert_matching(struct matchings *ms, struct matching *m){
...
int reinsert_matching(struct matchings *ms, struct matching *m){
  ...
  if (table != 0) {
    ...
    do {
      i = ( m->hash + j++ * second_hash ) % k;
    } while (table != 0);
  }

  memcpy(table[type] + i, m, sizeof(struct matching));
}
...
void reload_matchings(struct matchings *ms){
  ...
  for (int i = 0; i < ms->sizes; i++) {
    struct mapping *m = ms->m + i;
    if (m->hash != 0) {
      reinsert_matching(new_ms, m); 
    }
  }

  struct mapping *old_ms = ms->m;
...
char *match_in_dir(const char *dir_name, struct mappings mappings, int type){
  nftw(dir_namge, _match_in_dir, 30, 0); 
}
